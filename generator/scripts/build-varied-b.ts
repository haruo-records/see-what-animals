...
import { CATEGORIES_B } from "../varied/categories-b";

const OUT = join("generated", "varied-b");
const SCHEMES = ["teal", "ochre", "moss", "slate", "coral", "clay", "rose", "violet", "sky", "olive", "pine", "orchid"];

function main() {
  const seed = process.argv[2] ?? "b1";
  const grey = process.argv.includes("--grey");
  mkdirSync(OUT, { recursive: true });
  const rng = createRng(seed);
  const schemes = rng.shuffle([...SCHEMES]);

  CATEGORIES_B.forEach((cat, i) => {
    const built = cat.build(rng.fork(`catb-${cat.n}`));
    const form: Form = {
